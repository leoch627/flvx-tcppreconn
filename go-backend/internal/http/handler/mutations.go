...
func (h *Handler) nodeInstall(w http.ResponseWriter, r *http.Request) {
	...
	installURL := h.buildRawMainURL("install.sh")
	enabled, proxyURL := h.getGithubProxyConfig()

	var cmd string
	if enabled {
		cmd = fmt.Sprintf("curl -L %s -o ./install.sh && chmod +x ./install.sh && PROXY_ENABLED=true PROXY_URL=%s ./install.sh -a %s -s %s",
			installURL, proxyURL, processServerAddress(panelAddr), secret)
	} else {
		cmd = fmt.Sprintf("curl -L %s -o ./install.sh && chmod +x ./install.sh && PROXY_ENABLED=false ./install.sh -a %s -s %s",
			installURL, processServerAddress(panelAddr), secret)
	}
...
func (h *Handler) forwardCreate(w http.ResponseWriter, r *http.Request) {
	...
	forwardID, err := h.repo.CreateForwardTx(userID, userName, name, tunnelID, remoteAddr, defaultString(asString(req["strategy"]), "fifo"), now, inx, entryNodes, port, inIp, nullableInt(speedID), asInt(req["tcpPreconn"], 0))
	if err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
	...
	err = h.syncForwardServices(createdForward, "UpdateService", true)
	if err != nil {
		_ = h.deleteForwardByID(forwardID)
		response.WriteJSON(w, response.ErrDefault(err.Error()))
		return
	}
	writeForwardMutationSuccess(w, nil)
}
...
func (h *Handler) forwardUpdate(w http.ResponseWriter, r *http.Request) {
	...
	now := time.Now().UnixMilli()
	tcpPreconn := forward.TCPPreconn
	if _, hasTCPPreconn := req["tcpPreconn"]; hasTCPPreconn {
		tcpPreconn = asInt(req["tcpPreconn"], 0)
	}
	if err := h.repo.UpdateForward(id, name, tunnelID, remoteAddr, strategy, now, newSpeedID, tcpPreconn); err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
	if hasInIP {
	...
	if _, err := h.listForwardPorts(id); err != nil {
		h.rollbackForwardMutation(forward, oldPorts)
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
	...
	writeForwardMutationSuccess(w, warnings)
}

func writeForwardMutationSuccess(w http.ResponseWriter, warnings []string) {
	if len(warnings) == 0 {
		response.WriteJSON(w, response.OKEmpty())
		return
	}
...
func (h *Handler) forwardBatchChangeTunnel(w http.ResponseWriter, r *http.Request) {
	...
	for _, id := range req.ForwardIDs {
		...
		if _, portsErr := h.listForwardPorts(id); portsErr != nil {
			h.rollbackForwardMutation(forward, oldPorts)
			fail++
