...
func (w *WebSocketReporter) handleUpgradeAgent(data interface{}) error {
	...
	var baseReq struct {
		...
		SourceUpgrade bool   `json:"sourceUpgrade"`
	}
	if err := json.Unmarshal(jsonData, &baseReq); err != nil {
		return fmt.Errorf("解析升级参数失败: %v", err)
	}

	if baseReq.SourceUpgrade {
		return w.handleSourceUpgrade(jsonData)
	}
...
func (w *WebSocketReporter) handleSourceUpgrade(jsonData []byte) error {
	...
	if err := json.Unmarshal(jsonData, &req); err != nil {
		return fmt.Errorf("解析源码升级参数失败: %v", err)
	}
	...
	for _, ch := range branch {
		if !((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9') || ch == '-' || ch == '_' || ch == '.') {
			return fmt.Errorf("分支名包含非法字符")
		}
	}
	repoCloneURL := req.RepoCloneURL
	if repoCloneURL == "" {
		return fmt.Errorf("仓库克隆地址不能为空")
	}
	// Validate clone URL to prevent command injection
	if !strings.HasPrefix(repoCloneURL, "https://") && !strings.HasPrefix(repoCloneURL, "http://") {
		return fmt.Errorf("仓库克隆地址必须以 http:// 或 https:// 开头")
	}

	w.sendUpgradeProgress("downloading", 0, "开始从源码升级...")
	fmt.Printf("📦 开始从源码升级: %s (branch: %s)\n", repoCloneURL, branch)

	const srcDir = "/opt/flvx-agent-src"
	const binaryPath = "/etc/flux_agent/flux_agent"
	const tmpPath = "/etc/flux_agent/flux_agent.new"
	const backupPath = "/etc/flux_agent/flux_agent.old"

	// Pull latest source code
	w.sendUpgradeProgress("downloading", 10, "拉取最新源码...")
	if _, err := os.Stat(srcDir + "/.git"); err == nil {
		// Existing source directory: fetch and reset
		fetchCmd := exec.Command("git", "-C", srcDir, "fetch", "--all")
		if output, err := fetchCmd.CombinedOutput(); err != nil {
			return fmt.Errorf("拉取源码失败 (fetch): %v\n%s", err, string(output))
		}
		resetCmd := exec.Command("git", "-C", srcDir, "reset", "--hard", "origin/"+branch)
		if output, err := resetCmd.CombinedOutput(); err != nil {
			return fmt.Errorf("拉取源码失败 (reset): %v\n%s", err, string(output))
		}
	} else {
		// Fresh clone
		cloneCmd := exec.Command("git", "clone", "--depth", "1", "-b", branch, repoCloneURL, srcDir)
	...
	w.sendUpgradeProgress("downloading", 90, "编译完成")
	...
	// Also build tcp_pool if gcc is available
	const tcpPoolSrc = "/opt/flvx-agent-src/go-gost/tcp-preconn/tcp_pool.c"
	if _, err := os.Stat(tcpPoolSrc); err == nil {
		if gccPath, err := exec.LookPath("gcc"); err == nil {
			const tcpPoolPath = "/etc/flux_agent/tcp_pool"
			const tcpPoolTmp = "/etc/flux_agent/tcp_pool.new"
			// Try static link first, fall back to dynamic
			buildTcp := exec.Command(gccPath, "-O2", "-static", "-o", tcpPoolTmp, tcpPoolSrc, "-lpthread")
			if err := buildTcp.Run(); err != nil {
				buildTcp = exec.Command(gccPath, "-O2", "-o", tcpPoolTmp, tcpPoolSrc, "-lpthread")
				_ = buildTcp.Run()
			}
			if fi, err := os.Stat(tcpPoolTmp); err == nil && fi.Size() > 0 {
				if err := os.Chmod(tcpPoolTmp, 0755); err != nil {
					fmt.Printf("⚠️ tcp_pool chmod failed: %v\n", err)
				}
				if err := os.Rename(tcpPoolTmp, tcpPoolPath); err != nil {
					fmt.Printf("⚠️ tcp_pool rename failed: %v\n", err)
				}
			}
		}
	}

	// Replace binary and restart via systemd-run to avoid killing ourselves
	w.sendUpgradeProgress("installing", 95, "准备重启...")
	restartCmd := exec.Command("systemd-run", "--quiet", "/bin/sh", "-c",
		"sleep 1 && systemctl stop flux_agent && mv "+tmpPath+" "+binaryPath+" && systemctl start flux_agent")
